...
function generate_build_config_mk() {
  ...
  local static_args=""
  if [ $3 -eq "static" ] ; then
      static_args=$CONFIGURE_ARGS_STATIC
  fi
  if [ $2 -eq "32" ] ; then
    ./Configure $CONFIGURE_ARGS $CONFIGURE_ARGS_32 $static_args
  elif [ $2 -eq "64" ] ; then
    ./Configure $CONFIGURE_ARGS $CONFIGURE_ARGS_64 $static_args
  else
  ...
  (
    ...
    echo "openssl_cflags_$static_suffix$2 := \\"
...
function import() {
  ...
  generate_build_config_mk ../build-config-static-32.mk 32 static
  generate_build_config_mk ../build-config-static-64.mk 64 static
