...
function generate_build_config_mk() {
  ...
  # Re-configure again for static flags
  if [ $2 -eq "32" ] ; then
    ./Configure $CONFIGURE_ARGS $CONFIGURE_ARGS_32 $CONFIGURE_ARGS_STATIC
  elif [ $2 -eq "64" ] ; then
    ./Configure $CONFIGURE_ARGS $CONFIGURE_ARGS_64 $CONFIGURE_ARGS_STATIC
  else
    die "Unsupported $2-bit arch"
  fi
  rm -f apps/CA.pl.bak crypto/opensslconf.h.bak
  mv -f crypto/opensslconf.h crypto/opensslconf-static-$2.h
  cp -f crypto/opensslconf-static-$2.h include/openssl/opensslconf-static-$2.h

  (grep -e -D Makefile | grep -v CONFIGURE_ARGS= | grep -v OPTIONS= | grep -v -e -DOPENSSL_NO_DEPRECATED) > $tmpfile

  declare -r cflags_static=$(filter_by_egrep "^-D" $(grep -e "^CFLAG=" $tmpfile))
  declare -r depflags_static=$(filter_by_egrep "^-D" $(grep -e "^DEPFLAG=" $tmpfile))
  rm -f $tmpfile
  ...
  (
    ...
    echo "openssl_cflags_$2 := \\"
    for cflag in $cflags $depflags; do
      echo "  $cflag \\"
    done
    echo ""

    echo "openssl_cflags_static_$2 := \\"
    for cflag in $cflags_static $depflags_static; do
      echo "  $cflag \\"
    done
    echo ""
  ) > $1
