...
function generate_build_config_mk() {
  ...
  declare -r cfgdir=config/generic-$2/include
  mkdir -p ../$cfgdir/crypto
  mkdir -p ../$cfgdir/openssl
  cp -f crypto/opensslconf.h ../$cfgdir/crypto
  cp -f crypto/opensslconf.h ../$cfgdir/openssl
  rm -f apps/CA.pl.bak crypto/opensslconf.h.bak crypto/opensslconf.h
