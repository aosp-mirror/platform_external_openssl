...
function generate_config_mk() {
  ...
  (
    ...
    echo "# To regenerate, modify openssl.config, then run:"
...
"
    common_defines=$(var_value ${prefix}_DEFINES)
    print_defines_in_mk common_c_flags $common_defines

    common_sources=$(var_value ${prefix}_SOURCES)
    print_vardef_in_mk common_src_files $common_sources

    for arch in $all_archs; do
      arch_defines=$(var_value ${prefix}_DEFINES_${arch})
      print_defines_in_mk ${arch}_c_flags $arch_defines

      arch_sources=$(var_value ${prefix}_SOURCES_${arch})
      print_vardef_in_mk ${arch}_src_files $arch_sources

      arch_exclude_sources=$(var_value ${prefix}_SOURCES_EXCLUDES_${arch})
